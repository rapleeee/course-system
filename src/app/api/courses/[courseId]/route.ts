...
export async function GET(req: NextRequest, context: CourseRouteContext) {
  ...
  try {
    ...
    const chapterSnap = await courseRef.collection("chapters").orderBy("createdAt", "asc").get();
    const chapters = chapterSnap.docs.map((doc) => serializeChapter(doc.id, doc.data()));
